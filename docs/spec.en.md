...
- `wrapper: false` leaves out u8g2's `lgfx::U8g2font` declaration and names the byte array itself the font symbol (the upstream u8g2 shape, usable with `u8g2.setFont()`). Defaults to `true`; other formats ignore it.
- The generated header includes no graphics library header of its own. Include LovyanGFX (or M5GFX / M5Unified) — Adafruit GFX for a plain GFXfont — before it. VLW/BFF headers are data only and need nothing beyond `<stdint.h>`.
