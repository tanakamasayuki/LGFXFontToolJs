...
- `wrapper: false` は u8g2 の `lgfx::U8g2font` 宣言を出さず、バイト配列そのものをフォント記号にする（上流 u8g2 の形。`u8g2.setFont()` で使える）。既定は `true`。他の形式では無視する。
...
await generateFont({
  ...
  fallbacks: [{ family }], // 主ソースに無い文字をこの順で補完（同じ em で描く）
})  // -> { font, missing, filled }
```
...
- **補完の役割分担（決定済み）**: 生成時の補完（不足文字を別ソースで主書体と同じ `em` / 閾値のままラスタライズし、ベースライン整列で重ねて全グリフから行ボックスを再計算）は**ライブラリ機能**（`fallbacks`。何で何を埋めたかは `filled`、なお無いものは `missing`）。**すべての書体を同じ `em` で描くので、尺度は構造的に揃う**（以前は測定値を引き継ぐ必要があった）。どのソースで埋めるかの**選定と入手はアプリ責務**（Generator は FALLBACK_CHAIN を提案し、勝手には埋めない）。既存ビットマップフォント同士の補完は**ライブラリでは自動化しない** — `coverage → subset → merge` のレシピとし、ピクセルサイズの適合は利用者が判断する（使い方ガイドに記載）。
- ブラウザ以外で呼ぶと `CapabilityError('RASTERIZER_UNAVAILABLE')`。Node 対応はラスタライザ注入インタフェースとして将来検討する（§18）。

### 10.3 サイズは em で指定する

**`em` は書体のデザインサイズ（em 正方形）を画素で表したもの**で、CSS の `font-size` に
そのまま渡す値である。

- **全角 1 文字の送り幅がちょうど `em` になる。** 定義は縦ではなく**横**
- **行の高さは `em` より大きくなるのが普通。** 上下の余白と、`|` や括弧のように
  em を超えて伸びる字があるため。行ボックスは生成結果（`font.ascent` / `font.descent`）から読む

実測（Noto Sans JP）:

| `em` | 全角の送り幅 | 行の高さ | 最大の墨面 |
| --- | --- | --- | --- |
| 8 | **8** | 10 | 10 |
| 12 | **12** | 14 | 14 |
| 16 | **16** | 19 | 18 |
...
同梱ビットマップフォントの命名とも一致する（`lgfxJapanGothic_16` は全角の送り幅が 16）。

#### 測定に基づく尺度を採らない理由

以前は「基準字（`[漢 国 日 가 H E N 0]` のうち**要求した集合に入っている**もの）の墨面の
高さ」を尺度にし、二分探索で `cssPx` を追い込んでいた。これには 2 つの欠陥があった。

- **文字を 1 字足すと既存の字が縮む。** ASCII 95 字に `日` を足すと基準字が `H` → `日` に
  変わり、**既存 92 字の寸法と行の高さが変わった**（実測）
- **指定値が何も保証しない。** `16` と指定しても最大の墨面が 24 画素、行の高さが 25 画素に
  なりうる（基準字が最大の字とは限らないため）

`em` は測定を伴わないので、どちらも起きない。ASCII 95 字に漢字 7 字を足しても
**変化した字は 0/95**（実測）。この変更で `measureTtf` / `pickProbe` /
`PROBE_CANDIDATES` と `sizing` の引き回しがすべて不要になった。

### 10.4 決定性

同じ入力から同じフォントが出ることを生成の要件とする。

- 閾値処理・メトリクス丸め・グリフ境界の切り出しは、本ライブラリ内の**決定的なコード**で行う。設定（px、bpp、閾値、太さ）が同じなら出力は入力フォントとブラウザにのみ依存する。
- ブラウザエンジン間・OS 間でアンチエイリアスの結果は異なりうる。これは許容する（§10.1 — 生成物がそのまま正解になるため、再現性が必要な場面では生成物自体を成果物として保存する）。この性質はドキュメントに明記する。
- 生成した `Font` の `meta` に生成条件（元ファイル名、px、bpp、閾値、UA）を記録する。
...
| **Generator** | TTF → u8g2 / GFXfont / BDF / VLW / BFF。fontgen.html の UI を手本にした 4 ステップのカード式（書体 → サイズと名前 → 文字 → 生成。2026-08 リデザイン済み）。Google Fonts の再配布可能書体（OFL / Apache-2.0）からの検索・選択、生成前のライブプレビュー、テンプレート・軸別の文字集合選択と概算サイズ、文字一覧（Ctrl+F 可）、欠落文字の別書体からの補完（**提案 → 1 クリック適用**。不足分だけ生成して `merge`、候補は FALLBACK_CHAIN、帰属表示は両書体を記録）、C ソース / BDF テキスト / バイナリ出力 — いずれも実装済み。フォント取得のネットワークアクセスはアプリ側の責務（§2.3）で、ライブラリは `generateFont` に読み込み済みファミリと `fallbacks` を渡せる口だけ持つ | `generateFont` / `subset` / `merge` / `encode` / `encodeCSource` |
