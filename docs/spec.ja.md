...
| BFF | LovyanGFX（新しく未文書。カーニングあり） | 1–8 | 検討 | 検討 | 4 |
| fontx2 | 日本語組込みの古参形式（FatFs 界隈） | 1 | 検討 | 検討 | 4 |
...
| **Generator** | TTF → u8g2 / GFXfont。文字集合選択、閾値プレビュー、C ソース出力 | `generateFont` / `subset` / `merge` / `encode` / `encodeCSource` |
