 ...
 * @property {number} [filled] - how many characters came from this typeface
 * @property {Attribution[]} [fallbacks] - typefaces that supplied the rest, in order
 * @property {string} [command] - the command that produced the file, for reproducing it
 */

/**
 * One typeface's provenance. Every typeface a glyph came from needs its own
 * block: a fallback contributes a derived work just as the primary does, and
 * its license may differ.
 * @param {Attribution} a
 * @param {Font | null} font - only the primary source falls back to font metadata
 * @param {string} indent
 */
function typefaceLines(a, font, indent) {
  /** @type {string[]} */
  const L = [];
  /** @param {string} s */
  const put = (s) => L.push(indent + s);
  put(`Typeface : ${a.typeface || font?.familyName || '(unknown)'}`);
  if (a.author) put(`Author   : ${a.author}`);
  if (a.license || font?.meta.license) {
    put(`License  : ${a.license ?? font?.meta.license}`);
    if (a.licenseUrl) put(`           ${a.licenseUrl}`);
  } else {
    put('License  : UNKNOWN — supplied as a local file.');
    put('           You are responsible for confirming that this typeface may be');
    put('           embedded and redistributed in compiled firmware.');
  }
  if (a.origin) put(`Obtained : ${a.origin}`);
  ...
  if (a.sourceHash) put(`Source   : sha256:${a.sourceHash}`);
  return L;
}
...
export function licenseNotice(font, info) {
  ...
  L.push('');
  L.push(...typefaceLines(a, font, ''));
  if (font.meta.copyright) L.push(`Copyright: ${font.meta.copyright}`);
  const fallbacks = a.fallbacks ?? [];
  for (const fb of fallbacks) {
    L.push('');
    L.push(
      fb.filled === undefined
        ? 'Characters the typeface above does not have were taken from:'
        : `${fb.filled} character(s) the typeface above does not have were taken from:`,
    );
    L.push(...typefaceLines(fb, null, '  '));
  }
  L.push('');
  const many = fallbacks.length > 0;
  L.push(`This file contains glyphs rasterized from the typeface${many ? 's' : ''} above and is a`);
  L.push(`derived work of ${many ? 'them' : 'it'}. Keep this notice with the file and with any binary`);
  ...
  // How to make this file again. Paths outside the working directory are reduced
  // to a file name by the caller, so the line neither leaks nor depends on one
  // machine's layout — a local font may have to be put back by hand.
  if (a.command) {
    L.push('');
    // --out carries a file name and no directory: see REPRO_FLAGS in bin/lgfx-font-tool.js.
    L.push('Rebuild with:');
...
const HEADER_COMMENTS = {
  ...
  ja: {
    ...
    u8g2Raw:
      'データのみで、標準的な u8g2 フォント配列です。コンパイルにライブラリのヘッダは要りません。',
    gfxRange:
  ...
  'zh-Hans': {
    ...
    u8g2Raw: '仅为数据，是标准的 u8g2 字体数组。编译时无需包含库头文件。',
    gfxRange:
  ...
  'zh-Hant': {
    ...
    u8g2Raw: '僅為資料，是標準的 u8g2 字型陣列。編譯時無需包含程式庫標頭檔。',
    gfxRange:
 ...
 * Emits a u8g2 header.
 *
 * The byte array is the format the upstream u8g2 library reads. Only the
 * `lgfx::U8g2font` object wrapped around it is LovyanGFX-specific, so
 * `wrapper: false` drops that object and names the array itself `ident` —
 * the shape upstream u8g2 fonts have, usable with `setFont()` directly.
 * @param {Font} font
 * @param {string} ident
 ...
 */
function emitU8g2Header(font, ident, attribution, encodeOpts, language, wrapper) {
  const data = encodeU8g2(font, encodeOpts);
  const comments = headerComments(language);
  const guard = `LGFXFT_FONT_${ident.toUpperCase()}_H`;
  const array = wrapper ? `${ident}_data` : ident;
  let s = asComment(
    licenseNotice(font, {
      ident,
      format: wrapper ? 'u8g2 (1bpp) — lgfx::U8g2font' : 'u8g2 (1bpp) — data only',
      bytes: data.length,
      attribution,
    }),
  );
  ...
  s += `#ifndef ${guard}\n#define ${guard}\n\n`;
  s += '#include <stdint.h>\n\n';
  s += `// ${wrapper ? comments.u8g2Include : comments.u8g2Raw}\n\n`;
  s += PROGMEM_GUARD + '\n';
  s += `static const uint8_t ${array}[${data.length}] LGFXFT_PROGMEM = {\n`;
  s += hexTable(data, '  ');
  s += '};\n\n';
  if (wrapper) {
    s += `// ${comments.usage}:  display.setFont(&${ident});\n`;
    s += `static const lgfx::U8g2font ${ident}(${array});\n\n`;
  } else {
    s += `// ${comments.usage}:  u8g2.setFont(${array});\n\n`;
  }
  s += `#endif // ${guard}\n`;
  return s;
}
 ...
 * @param {boolean} [opts.wrapper] - u8g2: declare the lgfx::U8g2font object (default true)
 * @returns {string}
 */
export function encodeCSource(font, opts) {
  ...
  switch (opts.format) {
    case 'u8g2':
      return emitU8g2Header(font, ident, opts.attribution, encodeOpts, opts.language, opts.wrapper !== false);
    case 'gfx':
